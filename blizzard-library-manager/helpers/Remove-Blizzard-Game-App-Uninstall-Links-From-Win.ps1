...
foreach ($path in $registryPaths) {
    ...
    foreach ($subkey in $subkeys) {
        try {
            ...
            if ($uninstallString -like "*Blizzard Uninstaller*") {
                $keysFound++
...
Write-Host "  Blizzard registry keys found: $keysFound" -ForegroundColor White
Write-Host "  Blizzard registry keys deleted: $keysDeleted" -ForegroundColor White
if ($keysSkipped -gt 0) {
    Write-Host "  Blizzard registry keys skipped: $keysSkipped" -ForegroundColor White
}

if ($keysFound -eq 0) {
    Write-Host "No Blizzard uninstall registry keys found." -ForegroundColor Green
}
