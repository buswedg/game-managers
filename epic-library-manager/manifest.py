...
from utils.utils import read_json, save_json, backup_file
...
def _update_legendary_manifest(game):
    ...
    manifest_path = os.path.join(LEGENDARY_MANIFEST_PATH, 'installed.json')
    if not os.path.exists(manifest_path):
        logger.error(f"Legendary manifest not found: {manifest_path}")
        return False
